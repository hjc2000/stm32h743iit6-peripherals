...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
	...
	pcd->SetSofCallback([]()
						{
							USBD_LL_SOF(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
						});

	pcd->SetSetupStageCallback([](base::usb::fs_device_pcd::SetupStageCallbackArgs const &args)
							   {
								   USBD_LL_SetupStage(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(), const_cast<uint8_t *>(args.Span().Buffer()));
							   });

	pcd->SetResetCallback([]()
						  {
							  USBD_LL_SetSpeed(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(), USBD_SPEED_FULL);
							  USBD_LL_Reset(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
						  });

	pcd->SetSuspendCallback([]()
							{
								/* Inform USB library that core enters in suspend Mode. */
								USBD_LL_Suspend(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
	...
	pcd->SetResumeCallback([]()
						   {
							   ...
							   USBD_LL_Resume(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
						   });

	pcd->SetConnectCallback([]()
							{
								USBD_LL_DevConnected(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
							});

	pcd->SetDisconnectCallback([]()
							   {
								   USBD_LL_DevDisconnected(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle());
							   });

	pcd->SetDataOutStageCallback([](base::usb::fs_device_pcd::DataOutStageCallbackArgs const &args)
								 {
									 USBD_LL_DataOutStage(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(),
														  args.EndpointNumber(),
	...
	pcd->SetDataInStageCallback([](base::usb::fs_device_pcd::DataInStageCallbackArgs const &args)
								{
									// bsp::UsbFsDevicePcd::HalPcdHandle().IN_ep[args.EndpointNumber()].xfer_buff
									// 指向的是用户调用 USBD_LL_Transmit 时传入的缓冲区。
									USBD_LL_DataInStage(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(),
														args.EndpointNumber(),
														bsp::UsbFsDevicePcd::HalPcdHandle().IN_ep[args.EndpointNumber()].xfer_buff);
								});
	...
	pcd->SetIsoOutIncompleteCallback([](base::usb::fs_device_pcd::IsoOutIncompleteCallbackArgs const &args)
									 {
										 USBD_LL_IsoOUTIncomplete(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(),
																  args.EndpointNumber());
									 });

	pcd->SetIsoInIncompleteCallback([](base::usb::fs_device_pcd::IsoInIncompleteCallbackArgs const &args)
									{
										USBD_LL_IsoINIncomplete(&bsp::usb_cdc_serial_port_slot()[0]->UsbdHandle(),
																args.EndpointNumber());
									});
