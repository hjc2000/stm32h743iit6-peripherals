...
namespace bsp
{
	...
	private:
		...
		{
		public:
			handle_context(BaseTimer6 *self)
				: _self(self)
			{
			}
		...
		base::UsageStateManager<BaseTimer6> _usage_state_manager{};
	...
	public:
		...
		virtual void SetPeriodElapsedCallback(std::function<void()> const &callback) override
		{
			...
			base::interrupt::enable_interrupt(static_cast<int32_t>(IRQn_Type::TIM6_DAC_IRQn), 10);
		}
