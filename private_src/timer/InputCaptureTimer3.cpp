...
{
	if (_period_elapsed_callback == nullptr)
	{
		return;
	}

	_period_elapsed_callback();
}
	...
	case HAL_TIM_ActiveChannel::HAL_TIM_ACTIVE_CHANNEL_1:
		{
			...
			if (_capture_complete_callback_functions[channel_id - 1] == nullptr)
			{
				return;
			}

			base::input_capture_timer::CaptureCompleteEventArgs args{TIM3->CCR1};
			_capture_complete_callback_functions[channel_id - 1](args);
			break;
		}
...
void bsp::InputCaptureTimer3::SetPeriodElapsedCallback(std::function<void()> const &callback)
{
	...
	base::interrupt::disable_interrupt(static_cast<int32_t>(IRQn_Type::TIM3_IRQn));
	_period_elapsed_callback = callback;
...
void bsp::InputCaptureTimer3::SetCaptureCompleteCallback(uint32_t channel_id,
														 std::function<void(base::input_capture_timer::CaptureCompleteEventArgs const &)> const &callback)
{
	if (channel_id >= _capture_complete_callback_functions.size())
	{
		throw std::invalid_argument{CODE_POS_STR + "通道编号超出范围。"};
	}

	base::interrupt::disable_interrupt(static_cast<int32_t>(IRQn_Type::TIM3_IRQn));
	_capture_complete_callback_functions[channel_id - 1] = callback;
	base::interrupt::enable_interrupt(static_cast<int32_t>(IRQn_Type::TIM3_IRQn), 5);
}

void bsp::InputCaptureTimer3::StartChannel(uint32_t channel_id)
{
...
void bsp::InputCaptureTimer3::StopChannel(uint32_t channel_id)
{
